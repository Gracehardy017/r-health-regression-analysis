...
Sex has a p-value of 0.699641 and the p-values for the various regions have p-values above 0.01. These can be removed from the model. 
...
When comparing 2 people with all other variables the same, the average predicted charges grow by 23811.40 if the person is a smoker.
...
# 6 . Visualise 
```{r visualise bmi, echo = FALSE}
# Use ggplot to visually represent the correlation between bmi and charges, highlighting smokers vs non-smokers 
ggplot(insurance_df, aes( x = bmi, y = charges, color = smoker)) + geom_point() + geom_smooth(method = "lm", col = "blue") + labs(title = "Linear Regression of BMI on Charges", x = "BMI", y = "Charges")
```

```{r visualise age, echo = FALSE}
# Use ggplot to visually represent the correlation between age and charges, highlighting smokers vs non-smokers 
ggplot(insurance_df, aes( x = age, y = charges, color = smoker)) + geom_point() + geom_smooth(method = "lm", col = "blue") + labs(title = "Linear Regression of Age on Charges ", x = "Age", y = "Charges")
```

```{r visualise, echo = FALSE}
# Use ggplot to visually represent the correlation between no. of children and charges, highlighting smokers vs non-smokers 
ggplot(insurance_df, aes( x = children, y = charges, color = smoker)) + geom_point() + geom_smooth(method = "lm", col = "blue") + labs(title = "Linear Regression of No. of Children on Charges ", x = "No. of Children", y = "Charges")

```

# 7. Conclusion 

Age, BMI, and smoking status show strong statistical significance in predicting healthcare charges due to their small p-values.The linear regression is limited as Homoscedasticity and Normality assumptions were not met. 
